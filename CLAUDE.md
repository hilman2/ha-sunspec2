...
Work in this repo is intentionally sequenced into phases. The current state is **Phase 6 in progress** (v0.7.0 released 2026-04-07). All technical phases are done; the only remaining items are user-action: opening the HACS default-repo submission PR, and tagging v1.0.0 after a second user has tested the integration.
...
- **Phase 6 lint/format: every change must pass `ruff check` and `ruff format --check`** in CI. The pyproject.toml config is at the repo root. `mypy --strict` is intentionally NOT in CI yet because the private state (sensor.py class attributes, etc) is partially untyped. Adding type hints to make mypy --strict pass is a tracked follow-up for after the codebase has soaked.
- **Pytest pin `<9` is load-bearing.** Pytest 9.x changed the async-fixture handler in a way that breaks `pytest-homeassistant-custom-component`. Until the plugin updates, both the local WSL venv and the CI workflow must install `"pytest<9"`. Documented in REWRITE_PLAN.md Phase 1 findings.
