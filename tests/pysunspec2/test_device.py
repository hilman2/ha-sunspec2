import custom_components.sunspec2.pysunspec2.device as device
...
class TestGroup:
    ...
    def test__init_repeating_group_legacy_count_nested_group(self):
        # Regression test from sunspec/pysunspec2#120: a repeating group (Crv)
        # that itself contains a nested repeating group (Pt), as in model 705,
        # when the count must be inferred from the model length rather than
        # read directly from the count point (count == 0). Each Crv instance
        # is 10 registers of its own points plus 2 Pt points of 2 registers
        # each = 14 registers per curve; two curves make 28 repeating registers.
        gdef_crv = {
            "count": "NCrv",
            "name": "Crv",
            "groups": [
