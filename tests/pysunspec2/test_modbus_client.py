import custom_components.sunspec2.pysunspec2.modbus.client as client
import pytest
import socket
import tests.pysunspec2.mock_socket as MockSocket
import serial
import tests.pysunspec2.mock_port as MockPort
import custom_components.sunspec2.pysunspec2.file.client as file_client
import custom_components.sunspec2.pysunspec2.modbus.modbus as suns_modbus
import struct
...
class TestSunSpecModbusClientDeviceTCP:
    ...
    def test_connect(self, monkeypatch):
        # Upstream first connects for real to 127.0.0.1:502 and expects
        # 'Connection error'. The Home Assistant test plugin blocks every
        # real socket and fails the test at teardown for trying, so only
        # the mocked connect stays.
        d = client.SunSpecModbusClientDeviceTCP()
        monkeypatch.setattr(socket, 'socket', MockSocket.mock_socket)
...
class TestSunSpecFileClientDevice(object):
    ...
    def test_scan(self):
        d = file_client.FileClientDevice(filename='./tests/pysunspec2/test_data/device_1547.json', addr=40002)
        d.scan()
